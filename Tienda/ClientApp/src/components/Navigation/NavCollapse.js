...
import { get_all_category_by_id, save_all_category } from 'actions';
...
const NavCollapse = props => {
  ...
  const handleGetCategory = (event) =>{
    if(event.target.innerText == "Verduras"){
      dispatch(get_all_category_by_id(1));
    }else{
      dispatch(get_all_category_by_id(2));
    }
  ...
  const MenuItemChildren = isExpandable ? (
    <Collapse className='nav-collapse-item' in={open} timeout="auto">
      <List component="div" disablePadding onClick={handleGetCategory}>
        {
